-- Gaming Mouse Track V2.4 (Updated)
-- Fixes: Filter naming bug (_P11 -> _1), Dual Filter Logic (Crop + Scale)
-- Integrated Scaling/Aspect Ratio filter for stretching
-- Prevented filter stacking and improved zoom logic stability
...
function script_update(settings)
    ...
    for i = 1, count do
        ...
        if name and name ~= "" and name ~= "<None>" then
            ...
            if src then
                -- BUG FIX: Changed naming from "Track_P1" .. i to "Track_S" .. i
                local filter_name = "Track_S" .. i
                local scale_filter_name = "Stretch_S" .. i
...
function script_description()
    return "Gaming Mouse Tracking Screen V2.5.\n\nBUGS\n-Zooming have a odd effect"
end